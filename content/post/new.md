...
- 篇章结构全部重新调整，以全新的逻辑框架面世。“导读”一节推荐了三种读法，分别面向不同需求的读者。
- 除了新增一些图形之外，原书插图只要能改，就全部改为 ggplot 图形，并附有基础作图和 **ggplot2** 作图的两种源代码，含中文注释，并且全部经过 R 的最新版测试。
- 全面汉化，即书中的数据和图表里的英文元素尽最大可能进行了了中文化，既方便不习惯英文的读者，也提供了在图中添加汉字元素的方法。
- 在保持旧版原文风格的前提下，遣词造句、段落格式、乃至标点符号经过了作者和编辑再度精心打磨，更清晰流畅，易读易懂。
